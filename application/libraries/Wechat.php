...
class Wechat
{
	...
	public function __construct()
	{
        ...
        $token = $this->_ci->config->item('token');
        $appid =  $this->_ci->config->item('appid');
        $appsecret = $this->_ci->config->item('appsecret');
        $debug =  $this->_ci->config->item('debug');
        $logcallback = $this->_ci->config->item('logcallback');
        $partnerid =  $this->_ci->config->item('partnerid');
        $partnerkey = $this->_ci->config->item('partnerkey');
        $paysignkey =  $this->_ci->config->item('paysignkey');

        $this->token = !empty($token)? $token :'';
        $this->appid = !empty($appid)? $appid :'';
        $this->appsecret = !empty($appsecret)?$appsecret:'';
        $this->debug = !empty($debug)?$debug:false;
        $this->_logcallback = !empty($logcallback)?$logcallback:false;
        $this->partnerid  = !empty($partnerid)  ? $partnerid  : '';
        $this->partnerkey = !empty($partnerkey)  ? $partnerkey : '';
        $this->paysignkey = !empty( $paysignkey )  ?  $paysignkey  : '';
