...
<form id="form1" name="form1" method="post" action="add_rule">
    <table>
        ...
        <caption><h4>被添加自动回复设置</h4></caption>
        <input type="hidden" name="Id" id="Id" value="<?php if(!empty($model)) echo $model->Id; ?>">
        <tr>
           
            <td><input type="hidden" name="name" id="name"  value="sys_auto_subscribe" placeholder="请输入标题">
            <td><input type="hidden" name="message_type" id="message_type"  value="<?php  if(!empty($model)) echo $model->message_type; ?>">
